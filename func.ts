...
export async function updateUser(uid : string, type : string, admin : boolean, accept : boolean | null = null){
    const docRef = doc(firestore, "users", uid)
    if(type != "student"){
        updateDoc(docRef, {type: type, admin: admin})   
    } else updateDoc(docRef, {type: type, admin: admin, accept: accept == null ? deleteField() : accept })
}